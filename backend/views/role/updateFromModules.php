...
$this->title = Yii::t('maddoger/user', 'Update user roles from modules');
...
<div class="role-view">
    <div class="panel <?= $hasError ? 'panel-danger' : 'panel-success' ?>">
        <div class="panel-heading">
            <span class="panel-title"><?= Yii::t('maddoger/user', 'Log') ?></span> &nbsp;
            <?= Html::a(Yii::t('maddoger/user', 'Go back'), ['index'], ['class' => 'btn btn-xs btn-default']) ?>
        </div>
        <ul class="list-group">
        <?php
            foreach ($log as $item) {
